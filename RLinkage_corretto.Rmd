...
#OR = 1.53 Ciò significa che gli uomini hanno circa il 53% delle probabilità di morire rispetto alle donne, tenendo conto degli altri fattori nel modello.
```
...
summary(modello10)
#la prof non mette (link ="logit") non ho idea di che faccia
#io ho messo dentro tutte le variabili possibili
#ora provo a creare gli intervalli di confidenza
exp(cbind("Odds_Ratio" = coef(modello10), confint.default(modello10, level = 0.95)))
```
...
```{r}
#il modello di riferimento è quello del punto 9 con un OR di 1.53
library(epiR)
library(epitools)
library(foreign)
...
OR <- exp(modello$coefficients)
exp(cbind("OR" = coef(modello), confint.default(modello, level = 0.95)))

cont_table <- table(dead = colon$dead, 
                    sex = colon$sex, 
                    smoke = colon$smoke)
...
cont_table <- table(dead = colon$dead, 
                    sex = colon$sex, 
                    married = colon$work)
strat<-epi.2by2(dat=cont_table, method="cohort.count", interpret = FALSE)
strat$massoc.detail$OR.strata.wald
strat$massoc.detail$OR.homog.brday
strat
...
cont_table <- table(dead = colon$dead, 
                    sex = colon$sex, 
                    married = colon$education)
strat<-epi.2by2(dat=cont_table , method="cross.sectional") #chiedere al buon Liuk perchè in questo modo è l'unico dove homog.brday da un numero e NON null.
strat$massoc.detail$OR.strata.wald
strat$massoc.detail$OR.homog.brday
strat
...
cont_table <- table(dead = colon$dead, 
                    sex = colon$sex, 
                    married = colon$education)
strat<-epi.2by2(dat=cont_table , method="cohort.count") #chiedere al buon Liuk perchè in questo modo è l'unico dove homog.brday da un numero e NON null.
strat$massoc.detail$OR.strata.wald
strat$massoc.detail$OR.homog.brday
strat

#testo se sono confondenti

model_adj<-glm(dead ~ factor(sex) + factor(education),family=binomial,data=colon)
exp(cbind("OR" = coef(model_adj), confint.default(model_adj, level = 0.95)))

model_int<-glm(dead~ factor(sex)*factor(education),family=binomial,data=colon)
...
effetti_per_sesso <- predict(model_int, newdata = data.frame(sex = c("Female", "Male"), education = c("low", "medium/high")), type = "response")
effetto_maschi <- predict(model_adj, newdata = data.frame(sex = 'Male', education = colon$education))
effetto_femmine <- predict(model_adj, newdata = data.frame(sex = 'Female', education = colon$education))

effetto_per_istruzione <- aggregate(cbind(effetto_maschi, effetto_femmine), by = list(colon$education), FUN = mean)
...
colon[colon$age <= 14, "age_group"] <- "0-14"
colon[colon$age > 14 & colon$age <= 44, "age_group"] <- "15-44"
colon[colon$age > 44 & colon$age <= 64, "age_group"] <- "45-64"
colon[colon$age > 64, "age_group"] <- "> 64"

model <- glm(dead ~ yr2007, offset = log(person.years), 
             data = dat_swp[!is.na(dat_swp$yr2007), ], family = "poisson")
summary(model)
