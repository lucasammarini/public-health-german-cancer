...
##### 8. Stimare la sopravvivenza nei primi 5 anni dalla diagnosi per Stadio e effettuare un test d’ipotesi per verificare se l’azzardo di morte sia diverso per stadio di malattia alladiagnosi.
...
```{r}
cont_table <- table(sex = colon$sex,
                    dead = relevel(colon$dead, "1"),
                    genetic = colon$geneticm)
strat<-epi.2by2(dat=cont_table , method="cohort.count")
strat$massoc.detail$RR.strata.wald
strat$massoc.detail$OR.strata.wald
strat$massoc.detail$wRR.homog
strat$massoc.detail$wOR.homog
strat

cont_table <- table(genetic = relevel(colon$geneticm, "1")[colon$sex=="Male"],
                    dead = relevel(colon$dead, "1")[colon$sex=="Male"])
strat<-epi.2by2(dat=cont_table , method="cohort.count")
strat

cont_table <- table(genetic = relevel(colon$geneticm, "1")[colon$sex=="Female"],
...
cont_table <- table(education = relevel(colon$education, "medium/high")[colon$sex=="Male"],
                    dead = relevel(colon$dead, "1")[colon$sex=="Male"])
strat<-epi.2by2(dat=cont_table , method="cohort.count")
strat

cont_table <- table(education = relevel(colon$education, "medium/high")[colon$sex=="Female"],
                    dead = relevel(colon$dead, "1")[colon$sex=="Female"])
strat<-epi.2by2(dat=cont_table , method="cohort.count")
strat
...
model <- glm(dead ~ relevel(sex, "Male")+Stadio, data=colon, family="binomial")
summary(model)
model2 <- glm(dead ~ relevel(sex, "Male")*Stadio, data=colon, family="binomial")
summary(model2)
anova(model,model2,test="LRT")

model <- glm(dead ~ Stadio, data=colon[colon$sex=="Male",], family="binomial")
summary(model)
exp(cbind("Odds ratio" = coef(model), confint.default(model, level = 0.95)))

model <- glm(dead ~ Stadio, data=colon[colon$sex=="Female",], family="binomial")
summary(model)
exp(cbind("Odds ratio" = coef(model), confint.default(model, level = 0.95)))

# model <- glm(dead ~ Stadio, data=colon[colon$sex=="Male",], family=binomial(link="log"), maxit = 100)
# exp(cbind("Relative risk" = coef(model), confint.default(model, level = 0.95)))
# 
# model <- glm(dead ~ Stadio, data=colon, family=binomial(link="log"), maxit = 100)
# exp(cbind("Relative risk" = coef(model), confint.default(model, level = 0.95)))
# 
# model <- glm(dead ~ Stadio, data=colon[colon$sex=="Female",], family="binomial"(link="log"), maxit = 100)
# exp(cbind("Relative risk" = coef(model), confint.default(model, level = 0.95)))

model <- glm(as.numeric(dead) ~ relevel(sex, "Male") + as.factor(age), data=colon, family="poisson")
summary(model)
model2 <- glm(as.numeric(dead) ~ relevel(sex, "Male")*as.factor(age), data=colon, family="poisson")
summary(model2)
anova(model,model2,test="LRT")
