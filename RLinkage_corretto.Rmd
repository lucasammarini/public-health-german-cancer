...
Il test sull'omogeneità dei Rischi Relativi è significativo: la variabile potrebbe essere un modificatore di effetto per il RR.
...
model_male <- glm(dead ~ Stadio, data=colon[colon$sex=="Male",], family="binomial")
summary(model_male)
exp(cbind("Odds ratio" = coef(model_male), confint.default(model_male, level = 0.95)))

model_female <- glm(dead ~ Stadio, data=colon[colon$sex=="Female",], family="binomial")
summary(model_female)
exp(cbind("Odds ratio" = coef(model_female), confint.default(model_female, level = 0.95)))
...
Gli uomini hanno un OR maggiore tra Stadio 1 e 2 e tra Stadio 1 e 3 rispetto alle donne. Hanno invece un OR minore tra Stadio 1 e 4 rispetto alle donne.
...
Si osserva che l'interazione non è significativa: age non è un modificatore di effetto. BISOGNA STIMARE SE È UN CONFONDENTE CONFRONTANDO GLI OR GREZZO E DI MH.
