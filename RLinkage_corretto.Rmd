...
Il test ha un p-value inferiore a 2e-16, dunque si rigetta l'ipotesi nulla: la differenza tra le curve è statisticamente significativa.
...
```{r}
modello <- glm(dead ~ sex, data = colon, family = binomial())
summary(modello)
exp(cbind("OR" = coef(modello), confint.default(modello, level = 0.95)))
```
...
La prima variabile studiata è il fattore genetico.
...
Valitiamo i RR separatamente per sesso.
...
L'effetto del fattore genetico negli uomini è maggiore rispetto alle donne: l'interazione è negativa.
